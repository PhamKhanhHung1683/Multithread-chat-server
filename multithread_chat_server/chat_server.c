...
void remove_client(int client, int *client_sockets, char **client_names, int *num_clients);
...
int process_op(int client) {
    if (check_join(client)) {
        if (op_client == -1) {
            // Da co operator client
            op_client = client;
            char *msg = "100 OK\n";
            send(client, msg, strlen(msg), 0);
        } else {
            // Chua co operator client
            char *msg = "300 Already have an operator\n";
            send(client, msg, strlen(msg), 0);
        }
    } else {
        char *msg = "999 Unknown error\n";
...
int process_kick(int client, char *buf) {
    if (check_join(client)) {
        if (client == op_client) {
            ...
            if (k < num_clients) {
                int target_client = client_sockets[k];
                char *kick_msg = "KICK\n";
                send(target_client, kick_msg, strlen(kick_msg), 0);
                remove_client(target_client, client_sockets, client_names, &num_clients);
            ...
            char *msg = "400 Permission denied\n";
            send(client, msg, strlen(msg), 0);
        }
    } else {
        char *msg = "999 Unknown error\n";
